//
// Created by LEI on 15/10/28.
//

#import "QinLogFormatter.h"
#import "QinLogFields.h"
...
@implementation QinLogFormatter {
...
- (id)init {
    if((self = [super init]))
    {
        // Use standard QinLogFields Delegate
        logglyFieldsDelegate = [[QinLogFields alloc] init];
...
- (NSString *)formatLogMessage:(DDLogMessage *)logMessage
{
    ...
    NSString *iso8601DateString =  [QinCommonUtil convertDateToString:[QinCommonUtil makeTimestamp]];//[self iso8601StringFromDate:(logMessage->_timestamp)];
    ...
    [logfields setObjectNilSafe:[NSString stringWithFormat:@"%@:%lu", filestring, (unsigned long)logMessage->_line] forKey:@"lineNumber"];
    
    // newlines are not allowed in POSTS to Loggly
    NSString *logMsg = [logMessage->_message stringByReplacingOccurrencesOfString:@"\n" withString:@" "];
    [logfields setObjectNilSafe:logMsg forKey:@"logMessage"];
    ...
    if ([mostOftenADict isKindOfClass:[NSDictionary class]]) {
        ...
        if (!inputJsonError && [jsondictForLogMsg count] > 0) {
            ...
            if (!self.alwaysIncludeRawMessage) {
                [logfields removeObjectForKey:@"logMessage"];
            }
        }
    }
